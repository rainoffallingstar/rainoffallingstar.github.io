...
+ SP142 assay (Ventana, AZ) #用于PD-L1表达免疫组化

+ 高纯度FFPET RNA分离试剂盒（Roche）#提取RNA

+ Qubit和安捷伦生物分析仪 #RNA数量和质量评估

+ TruSeq RNA Access technology (Illumina) # 生成全转录组数据

+ R/Bioconductor软件包GenomicAlignments

+ Foundation One T7 assay (Foundation Medicine Inc., Cambridge,
   MA, USA)
...
+ consensus NMF clustering (CRAN. R package version 0.22.0, Brunet et al., 2004)

+ the random forest machine learning algorithm (R package randomForest)

+ QuSAGE analysis (R/Bionconductor qusage v2.18.0)
...
base sequence (parameters: ‘-M 2 -n 10 -B 2 -i 1 -N 1 -w 200000 -E 1-pairmax-rna = 200000 –clip-overlap). _#underline[To quantify gene
...
全转录组数据通过TruSeq RNA Access technology (Illumina)生成，RNA序列首先与核糖体RNA序列比对以排除之，其他序列与人类基因组序列对比，_#underline[为了量化基因的表达水平，使用R/Bioconductor软件包GenomicAlignments提供的功能计算了映射到每个RefSeq基因外显子上的读数。原始计数根据基因长度进行了调整，使用每百万转录本（TPM）归一化测量基因长度，并随后进行对数转换。？]_
...
+ Angiogenesis: VEGFA, KDR, ESM1, PECAM1, ANGPTL4, CD34; T-effector: CD8A,EOMES, PRF1, IFNG, CD274

+ Fatty Acid Oxidation /AMP-activated protein kinase (FAO/AMPK): CPT2, PPARA, CPT1A, PRKAA2,PDK2, PRKAB1; Cell Cycle: CDK2, CDK4, CDK6, BUB1B, CCNE1, POLQ, AURKA, MKI67, CCNB2; 

+ Fatty Acid Synthesis (FAS)/Pentose Phosphate: FASN, PARP1, ACACA, G6PD, TKT, TALDO1, PGD; 

+ Stroma: FAP, FN1, COL5A1, COL5A2, POSTN, COL1A1,COL1A2, MMP2; 

+ Myeloid Inflammation: CXCL1, CXCL2, CXCL3, CXCL8, IL6, PTGS2; 

+ Complement Cascade: F2, C1S, C1R, CFB,C3; 

+ Omega Oxidation: CYP4F3, CYP8B1, NNMT, MGST1, MAOA, CYP4F11, CYP4F2, CYP4F12; 

+ snoRNA: SNORD38A, SNORD104,SNORD32A, SNORD68, SNORD66, SNORD100. 
